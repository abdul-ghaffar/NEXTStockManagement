...
export async function createOrder(order: OrderData) {
    ...
    try {
        ...
        // Insert into Sale table. Use columns that exist in your schema: ClientName, SaleDate, TotalAmount, AreaID (optional)
        ...
        const saleResult = await saleRequest.query(`
            INSERT INTO [dbo].[Sale] (ClientName, SaleDate, TotalAmount, AreaID)
            OUTPUT INSERTED.ID
            VALUES (@ClientName, @SaleDate, @TotalAmount, @AreaID)
        `);
